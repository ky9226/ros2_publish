 ...
 public:
  ...
  ImagePubNode()
      : Node("ros2_test_video_publisher_node"),
        video_path_("/home/sky/ros2_test_video_publisher/slow_one.mp4"),
    ...
    // Create QoS settings for the publisher
    rclcpp::QoS qos_profile(10);  // Depth of 10, adjust as needed
    qos_profile.reliable();  // Ensures reliable delivery
    qos_profile.keep_last(10);  // Keeps the last 10 messages

    image_pub_ = this->create_publisher<sensor_msgs::msg::CompressedImage>("image_raw/compressed", 1);
 ...
 private:
  ...
  void publish_frame(const cv::Mat& frame) {
    if (!frame.empty()) {
      ...
      msg->header.frame_id = "camera_frame";  // Set an appropriate frame_id if needed
      msg->format = "jpeg";
